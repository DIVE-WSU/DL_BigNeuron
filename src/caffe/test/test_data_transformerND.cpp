...
void FillBlob(const int label,vector<int>&shape,
   const bool unique_pixels, Blob<TypeParam>& blob) {
  blob.Reshape(shape);
  int size = blob.count();
  TypeParam* data = blob.mutable_cpu_data();
  for (int j = 0; j < size; ++j) {
    TypeParam d = unique_pixels ? j : label;
    data[j] =d;
  }
}

template <typename Dtype>
class DataTransformNDTest : public ::testing::Test {
 protected:
  DataTransformNDTest()
      : seed_(1701),
      num_iter_(10) {}
  int setting(const TransformationNDParameter transND_param){
    return 0;
  ...
  //   DataTransformerND<Dtype>* transformer =
  //       new DataTransformerND<Dtype>(transform_param, phase);
  ...
  virtual ~DataTransformNDTest() { }

  int seed_;
  int num_iter_;
};

TYPED_TEST_CASE(DataTransformNDTest, TestDtypes);
TYPED_TEST(DataTransformNDTest, TestEmptyTransform){
   TransformationNDParameter transform_param;
   transform_param.set_mirror(false);
   transform_param.set_padding(true);
   vector<int> shape(3,5);
   shape.insert(shape.begin(),1);
   shape.insert(shape.begin(),1);
   Blob<TypeParam> blob;
   bool unique_pixels =true;
   FillBlob<TypeParam>(2l,shape,
      unique_pixels, blob);
      TypeParam elemt= blob.cpu_data()[74];
   EXPECT_EQ(elemt, 74);
   DataTransformerND<TypeParam>* transformer = new DataTransformerND<TypeParam>(transform_param);
   Blob<TypeParam> trans_blob;

   vector<int> off_set(3,0);
   vector<int > crop_shape(3,3);
   transformer->Transform(&blob,
                   &trans_blob,
                   off_set,
                   crop_shape);
   EXPECT_EQ(trans_blob.cpu_data()[0], 0);
   EXPECT_EQ(trans_blob.cpu_data()[8], 12);
   EXPECT_EQ(trans_blob.cpu_data()[9], 25);
   EXPECT_EQ(trans_blob.cpu_data()[26], 62);
   ...
   off_set[2] =(1);
    transformer->Transform(&blob,
                    ...
                    off_set,
                    crop_shape);
    EXPECT_EQ(trans_blob.cpu_data()[0], 31);
    EXPECT_EQ(trans_blob.cpu_data()[8], 43);
    EXPECT_EQ(trans_blob.cpu_data()[9], 56);
    EXPECT_EQ(trans_blob.cpu_data()[26], 93);
    ...
    transformer->Transform(&blob,
                    ...
                    crop_shape);
    EXPECT_EQ(trans_blob.cpu_data()[0], 0);
    EXPECT_EQ(trans_blob.cpu_data()[8], 0);
    EXPECT_EQ(trans_blob.cpu_data()[9], 0);
    EXPECT_EQ(trans_blob.cpu_data()[26], 31);
    ...
    off_set[2] =(3);
     transformer->Transform(&blob,
                     ...
                     off_set,
                     crop_shape);
     EXPECT_EQ(trans_blob.cpu_data()[0], 3);
     EXPECT_EQ(trans_blob.cpu_data()[8], 0);
     EXPECT_EQ(trans_blob.cpu_data()[9], 28);
     EXPECT_EQ(trans_blob.cpu_data()[26], 0);



  //  for(int i=0;i<trans_blob.count();i++)
  //  std::cout <<"blob v = "<<trans_blob.cpu_data()[i]<<std::endl;
   //   DataTransformer<TypeParam>* transformer =
   //       new DataTransformer<TypeParam>(transform_param, TEST);


  // transform_param.set_pad_method(transform_param_ZERO);
}
